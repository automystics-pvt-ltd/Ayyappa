/**
 * Tests: Donation Receipt reads temple name from site settings
 *
 * Verifies via static source analysis that Receipt.tsx (money donation receipt)
 * does NOT use hardcoded temple-identity strings and instead sources all three
 * fields from the `useSiteSettings()` hook:
 *
 *   • hero_subtitle      — temple name displayed in the receipt header
 *   • temple_address     — address line displayed under the header name
 *   • footer_temple_name — issuer name shown in the receipt footer
 *
 ...
 * through to the donation receipt page.
 *
...
console.log("\n── useSiteSettings hook wired into Receipt ──");

assert(
  /import\s*\{[^}]*useSiteSettings[^}]*\}\s*from/.test(receiptSrc),
  "Receipt.tsx imports useSiteSettings"
);

assert(
  /useSiteSettings\s*\(\s*\)/.test(receiptSrc),
  "Receipt.tsx calls useSiteSettings() to obtain live settings"
);
...
assert(
  ...
  "Receipt.tsx renders s.hero_subtitle (header temple name from settings)"
);
...
assert(
  ...
  "Receipt.tsx uses s.hero_subtitle with a fallback (graceful when unset)"
);
...
assert(
  ...
  "Receipt.tsx renders s.temple_address (header address from settings)"
);

assert(
  /s\.temple_address\s*\|\|/.test(receiptSrc),
  "Receipt.tsx uses s.temple_address with a fallback (graceful when unset)"
);

// ──────────────────────────────────────────────────────────────────────────────
// 4. footer_temple_name — issuer name in the receipt footer
// ──────────────────────────────────────────────────────────────────────────────

console.log("\n── footer_temple_name → receipt footer issuer ──");

assert(
  /s\.footer_temple_name/.test(receiptSrc),
  "Receipt.tsx renders s.footer_temple_name (footer issuer from settings)"
);

assert(
  /s\.footer_temple_name\s*\|\|/.test(receiptSrc),
  "Receipt.tsx uses s.footer_temple_name with a fallback (graceful when unset)"
);
...
console.log(`\n${"─".repeat(60)}`);
if (failed === 0) {
  console.log(`✅  All ${passed} donation receipt settings binding checks passed.`);
  ...
  console.log(`    on the donation receipt page once the browser re-fetches settings.`);
} else {
  console.error(`❌  ${failed} check(s) failed, ${passed} passed.`);
  console.error(`    Receipt.tsx may be using hardcoded temple-identity strings`);
